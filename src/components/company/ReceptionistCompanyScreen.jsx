...
export default function ReceptionistCompanyScreen() {
    ...
    return (
        ...
        // <div style={{display:"flex",flexDirection:"row"}}>
             <Grid container spacing={3} style={{ margin: "10px", padding: "10px", display: 'flex', justifyContent: "space-between", }}>
          
          {companyName.map((company) => (
              <Grid item key={company.id} xs={12} sm={6} md={4} lg={3}>
                  <Card onClick={() => handleCompanyChange(company.id,company.name)} sx={{ height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'center',backgroundColor:"#32577e" }}>
                      <CardMedia
